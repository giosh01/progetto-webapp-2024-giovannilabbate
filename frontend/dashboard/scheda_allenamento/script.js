...
(async function () {
  ...
  async function renderGMChart(schedaId){
    ...
    const ctx = canvas.getContext('2d'); //oggetto che charta.js usa per disegnare
    const barColor = getComputedStyle(document.documentElement).getPropertyValue('--bar-gm').trim() // legge colore da CSS
    ...
    const dataset = {
      label:'Serie', data,
      backgroundColor: barColor, borderWidth:0, borderRadius:2,
